...
<nav class="navbar navbar-fixed-top navbar-inverse" role="navigation">
    <div class="container">
        <div class="navbar-header">
            <a  class="navbar-brand " style="color: red" href="<c:url value="/list/list.do?method=getAll"/>">电商系统,卖家系统-商品管理</a>
        </div>
    ...
    <div class="row row-offcanvas row-offcanvas-right">
        ...
        <div class="col-xs-6 col-sm-3 sidebar-offcanvas" id="sidebar" role="navigation">
            <div class="list-group">
                <c:forEach items="${requestScope.firstArticleTypes }" var="type">
                    <a href="<c:url value="/list/list.do?method=getAll&typeCode=${type.code }"/>" class="list-group-item <c:if test="${type.code eq param.typeCode or top.code eq fn:substring(param.typeCode, 0, 4) }">active</c:if>">${type.name }</a>
                </c:forEach>
            </div>
        </div>
        <!--  侧边导航结束 -->
        ...
        <div class="col-xs-12 col-sm-9">
            ...
            <div class="alert alert-info" role="alert">
                <div>
                    <form action="<c:url value="/list/list.do?method=getAll"/>" method="post">
         ...
         <c:forEach items="${articles }" var="a">
            	...
            	<span class="thumbnail">
                    <a class="label label-danger" href="<c:url value="/list/list.do?method=deleteById&id=${a.id }&typeCode=${typeCode}&secondType=${secondType}&title=${title}"/>">删除</a>
                    <a class="label label-success" href="<c:url value="/list/list.do?method=showUpdate&id=${a.id }&typeCode=${typeCode}&secondType=${secondType}&title=${title}"/>">修改</a>
				    <a href="<c:url value="/list/list.do?method=preArticle&id=${a.id }"/>">
            ...
            <div class="row"  >
                ...
                <nav>
                    <ul class="pagination">
                        <li><a href="${pageContext.request.contextPath}/list/list.do?method=getAll&typeCode=${typeCode}&secondType=${secondType}&title=${title}&pageIndex=1">首页</a></li>
                        <li><a href="${pageContext.request.contextPath}/list/list.do?method=getAll&typeCode=${typeCode}&secondType=${secondType}&title=${title}&pageIndex=${pager.pageIndex -1}">上一页</a></li>
                        <li><a href="${pageContext.request.contextPath}/list/list.do?method=getAll&typeCode=${typeCode}&secondType=${secondType}&title=${title}&pageIndex=${pager.pageIndex + 1}">下一页</a></li>
                        <li><a href="${pageContext.request.contextPath}/list/list.do?method=getAll&typeCode=${typeCode}&secondType=${secondType}&title=${title}&pageIndex=${pager.totalPages}">尾页</a></li>
                        <li>
...
<script type="text/javascript">
        ...
        $(function(){
            $("#secondType").change(function(){
                    window.location = "${pageContext.request.contextPath}/list/list.do?method=getAll&typeCode=${typeCode}&secondType="+this.value;
            }) ;
